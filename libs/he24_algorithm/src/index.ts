...
export const suan_list = [
  {
    label: CalcOpEnum.ADD,
    suan: (left: number, right: number) => {
      return left + right;
    },
...
export class SolutionNode {
  ...
  getPrintStr(needQuote?: boolean): string {
    ...
    if (!this.calc_print_str) {
      ...
      this.calc_print_str = mapStr(
        this.left.getPrintStr(left),
        this.right.getPrintStr(right),
        this.op.label,
      );
      if (needQuote) {
