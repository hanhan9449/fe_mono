...
class PageEvent {
  private _key_to_callback_list = new Map<
    PageEventName,
    PickPageEventCallback<PageEventName>[]
  >();
  subscribe<T extends PageEventName>(
    eventName: T,
    callback: PickPageEventCallback<T>,
  ) {
...
type PickPageEventCallback<T extends PageEventName> = (
  a: PageEventCallback[T],
) => void;
