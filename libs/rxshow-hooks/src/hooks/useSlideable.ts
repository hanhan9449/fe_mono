...
function makeSlideable$(
  el: Nullable<HTMLElement>,
  config: SlideableConfigType = defaultConfig,
) {
  if (!el) {
    return of();
  }
  ...
  const move$ = mousedown$.pipe(
    ...
    mergeScan(
      (prev, curr) => {
        ...
        const contentWidth =
          Number(vars.getPropertyValue("--content-width")) || 0;
        const contentHeight =
          Number(vars.getPropertyValue("--content-height")) || 0;
      ...
      {
        xPercent: Number(initVars.getPropertyValue("--x-percent")),
        yPercent: Number(initVars.getPropertyValue("--y-percent")),
      },
    ),
  );
  return move$;
