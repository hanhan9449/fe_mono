...
import {
  useDraggableReceiver,
  useLatestFn,
  useReceiverIdRef,
} from "@hanhan9449/rxshow-hooks";
...
export function DraggableReceiver<T extends any>(
  props: DraggableReceiverProps,
) {
  ...
  return (
    <div ref={ref} style={props.containerStyle}>
      {item && (
        <DraggableSender
          key={`${item.receiverId}@${item.itemId}@${item.itemType}`}
          sendItem={item}
        />
      )}
    </div>
